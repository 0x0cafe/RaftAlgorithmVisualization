...
define([], function () {
    return function (frame) {
        ...
        .after(1000, function () {
            frame.model().title = '<h2 style="visibility:visible">所以，什么是分布式共识？</h2>'
                        + '<h3 style="visibility:hidden;">让我们先从一个例子开始...</h3>'
        ...
        .after(1000, function () {
            frame.model().subtitle = '<h3>我们也有一个<span style="color:green">客户端</span>，它可以向服务端发送数据。</h3>'
                           + frame.model().controls.html();
            layout.invalidate();
        })
        .after(500, function () {
