...
define([], function () {
    return function (frame) {
        ...
        //------------------------------
        .then(function () {
            subtitle('<h2>一旦我们选出了leader，我们需要将对系统的所有更改复制到所有节点。</h2>', false);
        })
        .then(wait).indefinite()
        .then(function () {
            subtitle('<h2>这是通过使用用于心跳的相同<em>附加条目</em>消息完成的。</h2>', false);
        })
        .then(wait).indefinite()
        .then(function () {
        ...
        .then(function () {
            ...
            subtitle('<h2>首先，客户端向leader发送更改。</h2>', false);
        })
        .then(wait).indefinite()
        .then(function () {
        ...
        .after(1, function () {
            subtitle('<h2>Raft甚至可以在网络分区面前保持一致。</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
            subtitle('<h2>让我们添加一个分区，将A和B与C，D和E分开。</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
        ...
        .after(1, function () {
            subtitle('<h2>由于我们的分区，我们现在有两个不同的leader。</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
            model().clients.create("Y");
            subtitle('<h2>让我们添加另一个客户端并尝试更新两个leader。</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
        ...
        .after(1, wait).indefinite()
        .after(1, function () {
            subtitle('<h2>节点B无法复制到多数，因此其日志条目保持未提交状态。</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
            var leader = model().leader(["C", "D", "E"]);
            client("X").send(leader, "SET 8");
            subtitle('<h2>另一个客户端将尝试设置节点' + leader.id + '的值为"8".</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
            subtitle('<h2>这将成功，因为它可以复制到多数。</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
            subtitle('<h2>现在让我们恢复网络分区。</h2>', false);
        })
        .after(1, wait).indefinite()
        .after(1, function () {
        ...
        .after(1, function () {
            subtitle('<h2>节点B将看到更高的选举期限并下台。</h2>');
        })
        .after(1, function () {
            subtitle('<h2>节点A和B都将回滚其未提交的条目并匹配新的leader的日志。</h2>');
        })
        .after(1, wait).indefinite()
        .after(1, function () {
            subtitle('<h2>我们的日志现在在我们的集群中保持一致。</h2>', false);
        })
        .after(1, wait).indefinite()
