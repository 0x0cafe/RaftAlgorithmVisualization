...
var LeastSquares = React.createClass({
        ...
        if (this.props.mode === 'regression')
            ...
            .call(d3.behavior.drag()
                .on('drag', (d, i) => {
                    ...
                    p[0] = d3.round(p[0], 2), p[1] = d3.round(p[1], 2)
                    self._clamp(p)
                    self.props.onDragNob('regression', {
                        pos: p,
                        d: d,
                        i: i
                    })
    ...
    _updateStateFromProps(props, state) {
        ...
        if (props.mode === 'point') {
            ...
            reg = (function() {
                var x1 = props.regressionPoints[0][0],
                    y1 = props.regressionPoints[0][1]
                var x2 = props.regressionPoints[1][0],
                    y2 = props.regressionPoints[1][1]
                var dy = y2 - y1,
                    dx = x2 - x1
        ...
        state.errors = props.points.map(d => {
            var point = acc(d)
            return {
                err: rs(point[0]) - point[1] /* err = x - X */ ,
                d: d
            }
        })
        state.reg = reg
