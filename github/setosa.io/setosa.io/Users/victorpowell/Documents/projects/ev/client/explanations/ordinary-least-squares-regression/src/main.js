...
var App = React.createClass({
    ...
    _getBetas(points) {
        var X = points.map(function(d) {
            return [d.point[0]]
        })
        var y = points.map(function(d) {
            return d.point[1]
        })
    ...
    render() {
        ...
        src = '/ev/linear-regression/resources/point-tutorial.gif' / >
            ...
            /p> <
            p >
            Statistical regression is basically a way to predict unknown quantities from a batch of existing data.For example, suppose we start out knowing the height and hand size of a bunch of individuals in a "sample population,"
        and that we want to figure out a way to predict hand size from height
        for individuals not in the sample.By applying OLS, we 'll get an equation that takes hand size---the '
        independent ' variable---as an input, and gives height---the '
        dependent ' variable---as an output. <
            /p> <
            p >
            Below, OLS is done behind - the - scenes to produce the regression equation.The constants in the regression-- - called 'betas'-- - are what OLS spits out.Here, beta_1 is an intercept;
        it tells what height would be even
        for a hand size of zero.And beta_2 is the coefficient on hand size;
        it tells how much taller we should expect someone to be
        for a given increment in their hand size.Drag the sample data to see the betas change. <
            /p> <
            LeastSquares
        key = 'least-squares'
        points = {
            this.state.leastSquaresPoints
        }
        betas = {
            this.state.betas
        }
        ...
        /> <
        p >
            At some point, you probably asked your parents, "Where do betas come from?"
        Let 's raise the curtain on how OLS finds its betas. <
            /p> <
            p >
        ...
        leastSquaresValueAccessor = {
            d => d.error
        }
        leastSquaresColorAccessor = {
            d => d.d.color
        }
        /> <
